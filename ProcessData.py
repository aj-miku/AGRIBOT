...
def process_temperature_range(temp_range):
    if '-' in temp_range:
        ...
        return round((lower + upper) / 2, 2)  # Calculate average for the range and round to 2 decimal places
    else:
        return round(float(temp_range), 2)   # Convert single value to float
